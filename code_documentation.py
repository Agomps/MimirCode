...
def main():
    directory_to_scan = input("Enter the path to the directory containing your program files: ")

    if not os.path.isdir(directory_to_scan):
        print(f"Error: The provided path '{directory_to_scan}' is not a valid directory or does not exist.")
        return

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    print(f"Output documentation will be saved in: {os.path.abspath(OUTPUT_DIR)}")

    documented_files_info = []

    file_paths = scan_directory(directory_to_scan)
    ...
    for file_path in file_paths:
        relative_path_of_code_file = os.path.relpath(file_path, directory_to_scan)
        base_name = os.path.basename(file_path)
        file_name_without_ext, ext = os.path.splitext(base_name)
        file_type = get_file_type(ext)

        output_file_name = f"{file_name_without_ext}.md"
        output_sub_dir = os.path.join(OUTPUT_DIR, os.path.dirname(relative_path_of_code_file))
    ...
    generate_table_of_contents(documented_files_info, OUTPUT_DIR)

    print("\nDocumentation generation complete!")
    print(f"Check the '{OUTPUT_DIR}' directory for your generated Markdown files and the TABLE_OF_CONTENTS.md.")
